...
# Program throws an error unless we do this. obviously i'll have to change this later
...
if app.config["DB_REPL"] != "":
    connect(app.config["DB_DATABASE"], host=app.config["DB_HOST"], 
            port=app.config["DB_PORT"], replicaset=app.config["DB_REPL"], username=app.config["USERNAME"], password=app.config["PASSWORD"])
else:
    connect(app.config["DB_DATABASE"], host=app.config["DB_HOST"], 
